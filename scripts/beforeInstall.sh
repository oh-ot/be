...
if [[ "$(docker images -q jtu1102/spring-cicd-test:latest 2> /dev/null)" != "" ]]; then
docker rmi -f $(docker images --format '{{.Repository}}:{{.Tag}}' --filter=reference='jtu1102/spring-cicd-test:latest')
fi
